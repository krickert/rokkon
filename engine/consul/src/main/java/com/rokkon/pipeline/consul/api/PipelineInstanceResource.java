...
public class PipelineInstanceResource {
                ...
                if (result.valid()) {
                    ...
                    if (result.errors().stream().anyMatch(e -> e.contains("not found"))) {
                        ...
                        return Response.status(Status.CONFLICT)
                            .entity(Map.of("error", "Cannot delete running instance"))
                            .build();
                    } else {
                        return Response.status(Status.BAD_REQUEST)
                            .entity(Map.of("error", result.errors().get(0)))
                            .build();
                    }
                }
            });
     ...
     */
    @Schema(description = "Create pipeline instance request")
    public record CreateInstanceRequest(
        @Schema(description = "Instance ID", required = true, example = "prod-pipeline-1")
        String instanceId,
        
        @Schema(description = "Pipeline definition ID to instantiate", required = true, example = "document-processing")
        String pipelineDefinitionId,
        
        @Schema(description = "Human-readable name", example = "Production Document Processor")
        String name,
        
        @Schema(description = "Instance description", example = "High-throughput document processing")
        String description,
        
        @Schema(description = "Kafka topic prefix", example = "prod-docs")
        String kafkaTopicPrefix,
        
        @Schema(description = "Processing priority", example = "100", minimum = "1", maximum = "1000")
        Integer priority,
        
        @Schema(description = "Max parallel processing threads", example = "10", minimum = "1")
        Integer maxParallelism,
        
        @Schema(description = "Step configuration overrides")
    ...
    public record CreateInstanceResponse(
        ...
        @Schema(description = "Created instance ID")
        String instanceId,
