    ...
    public Uni<ValidationResult> deleteCluster(String clusterName) {
        ...
        return UniHelper.toUni(getConsulClient().deleteValues(key))
            ...
            .onFailure().recoverWithItem(error -> {
                ...
                return ValidationResult.failure("Failed to delete cluster: " + error.getMessage());
            });
    }
            ...
            return UniHelper.toUni(getConsulClient().putValue(key, json))
                .map(success -> {
                    if (Boolean.TRUE.equals(success)) {
                        ...
                        return ValidationResult.failure("Failed to create cluster in Consul");
                    }
                })
                .onFailure().recoverWithItem(error -> {
                    LOG.errorf(error, "Failed to store cluster metadata");
                    return ValidationResult.failure("Failed to store cluster metadata: " + error.getMessage());
                });
        } catch (Exception e) {
            LOG.error("Failed to serialize cluster metadata", e);
            return Uni.createFrom().item(ValidationResult.failure("Failed to serialize cluster metadata: " + e.getMessage()));
        }
    }
            ...
            return UniHelper.toUni(getConsulClient().putValue(key, json))
                .map(success -> {
                    if (Boolean.TRUE.equals(success)) {
                        ...
                        return ValidationResult.failure("Failed to create cluster config in Consul");
                    }
                })
                .onFailure().recoverWithItem(error -> {
                    LOG.errorf(error, "Failed to create initial cluster config");
                    return ValidationResult.failure("Failed to create cluster config: " + error.getMessage());
                });
        } catch (Exception e) {
            LOG.error("Failed to serialize initial cluster config", e);
            return Uni.createFrom().item(ValidationResult.failure("Failed to create cluster config: " + e.getMessage()));
        }
    }
}