# Pipeline Engine: Network Topology

The Pipeline Engine and its ecosystem of modules, along with dependencies like Consul and Kafka, can be deployed in various network setups. The topology depends on factors like scale, security requirements, and whether the deployment is on-premises, cloud-based, or hybrid.
...
2.  **Pipeline Modules (Connectors, Steps, Sinks):**
    *   Listen for gRPC requests from the Pipeline Engine (or other modules if direct gRPC communication is used) on their own gRPC port (e.g., `9090`).
    *   Connect to the Pipeline Engine's `ModuleRegistrationService` (gRPC) for registration (often via `pipeline-cli`).
...
3.  **Consul:**
    *   Listens for HTTP API requests (e.g., port `8500`) from the Pipeline Engine, modules, and administrative tools.
...
4.  **Kafka:**
    *   Listens for client connections (e.g., port `9092`) from the Pipeline Engine and modules.
    *   Kafka brokers communicate with each other and with ZooKeeper (if not using KRaft mode) in a cluster.

5.  **Frontend/CLI Users:**
    *   Connect to the Pipeline Engine's HTTP/S port for the web UI or REST API.

## Common Network Setups

### 1. Single-Host / Local Development Setup

*   **Description:** All components (Pipeline Engine, modules, Consul, Kafka) run on a single machine (e.g., a developer's laptop). Often uses Docker Compose.
...
graph TD
    subgraph "Developer Machine (e.g., Docker Host)"
        PipelineEngine[Pipeline Engine <br> HTTP:38090, gRPC:49000]
        Module1[Module 1 <br> gRPC:9090]
        Module2[Module 2 <br> gRPC:9091]
        ConsulServer[Consul <br> HTTP:8500]
        KafkaServer[Kafka <br> TCP:9092]

        PipelineEngine -- "localhost/docker_ip" --> ConsulServer
        PipelineEngine -- "localhost/docker_ip" --> KafkaServer
        PipelineEngine -- "localhost/docker_ip:9090" --> Module1
        PipelineEngine -- "localhost/docker_ip:9091" --> Module2

        Module1 -- "localhost/docker_ip:49000" --> PipelineEngine
    %% Registration
        Module2 -- "localhost/docker_ip:49000" --> PipelineEngine
    %% Registration
    ...
    User[User/Browser/CLI] -- "localhost:38090" --> PipelineEngine
    ...
    class PipelineEngine,Module1,Module2,ConsulServer,KafkaServer component;
    class User external;
```
...
*   **Networking:**
    *   Kubernetes DNS is used for service-to-service communication (e.g., `pipeline-engine-service.namespace.svc.cluster.local`).
    *   Consul service discovery still plays a key role, especially for discovering individual module pod IPs and their health.
    *   External access to the Pipeline Engine UI/API is managed via Kubernetes Ingress or LoadBalancer services.
...
graph TD
    subgraph "Kubernetes Cluster"
        subgraph "Namespace: pipeline"
            PipelineEngineService[K8s Service: pipeline-engine] --> PipelineEnginePods["Pipeline Engine Pods <br> (Deployment)"]
        ...
        Module1Pods -- K8s DNS: pipeline-engine.pipeline... --> PipelineEnginePods
        Module2Pods -- K8s DNS: pipeline-engine.pipeline... --> PipelineEnginePods

        %% Engine to Module communication (via Consul discovered Pod IPs or K8s Service for module type)
        PipelineEnginePods -- Pod IPs (via Consul) or K8s Service --> Module1Pods
        PipelineEnginePods -- Pod IPs (via Consul) or K8s Service --> Module2Pods
        ...
        %% External Access
        Ingress[Kubernetes Ingress/LoadBalancer] --> PipelineEngineService
    end
    ...
    class PipelineEngineService,Module1Service,Module2Service,ConsulService,KafkaService k8sService;
    class PipelineEnginePods,Module1Pods,Module2Pods,ConsulPods,KafkaPods k8sPods;
    class Ingress,User external;
```

### 3. Multi-Cluster / Hybrid Deployment (Advanced)

*   **Description:** Pipeline Engine might run in one cluster/VPC, while modules or data services (Kafka/Consul) run in different clusters, VPCs, or even on-premises.
*   **Networking:**
...
2.  **Encryption in Transit:**
    *   **TLS for all HTTP traffic:** Pipeline Engine UI/API, Consul API.
...
Choosing the right network topology involves balancing performance, scalability, security, cost, and operational complexity. The Pipeline Engine's design, relying on established tools like gRPC, Consul, and Kafka, provides the flexibility to adapt to a wide range of these requirements.
