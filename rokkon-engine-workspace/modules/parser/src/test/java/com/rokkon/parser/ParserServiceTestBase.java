...
public abstract class ParserServiceTestBase {
    ...
    void testGetServiceRegistration() {
        var registration = getParserService().getServiceRegistration(Empty.getDefaultInstance())
                .subscribe().withSubscriber(UniAssertSubscriber.create())
                .awaitItem()
                .getItem();
        
        assertThat(registration.getModuleName()).isEqualTo("parser-module");
        // Parser service may have JSON schema in the future
        // For now, just verify the module name is correct
    }
}