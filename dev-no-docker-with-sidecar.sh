...
cleanup_consul_registrations() {
    echo -e "\n${BLUE}3️⃣  Cleaning up old service registrations...${NC}"
    
    # Clean via agent (not server)
    local services=$(curl -s "http://localhost:$CONSUL_AGENT_PORT/v1/agent/services" | jq -r 'to_entries[] | select(.value.Service == "pipeline-engine") | .key')
    
    if [ -n "$services" ]; then
        echo "Found old registrations to clean up:"
        for service_id in $services; do
            echo "  - Deregistering: $service_id"
            curl -s -X PUT "http://localhost:$CONSUL_AGENT_PORT/v1/agent/service/deregister/$service_id"
        done
