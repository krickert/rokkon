...
/**
 * Comprehensive unit tests for EmbedderService using Mockito for mocking dependencies.
...
class EmbedderServiceUnitTest {
    ...
    void testProcessDataWithValidChunks() throws Exception {
        ...
        lenient().when(vectorizer.isUsingGpu()).thenReturn(true);
    ...
    @Test
    void testEmbedderOptionsNullSafety() throws Exception {
        PipeDoc inputDoc = PipeDoc.newBuilder()
                .setId("test-doc-null-safety")
                .setBody("Test content")
                .build();
        ...
        // Create EmbedderOptions with null documentFields to test null safety
        EmbedderOptions optionsWithNulls = new EmbedderOptions(
                null, // embeddingModels = null
                null, // checkChunks = null  
                true, // checkDocumentFields = true
                null, // documentFields = null (this was causing NPE)
                null, // customFieldMappings = null
                null, // processKeywords = null
                null, // keywordNgramSizes = null
                null, // maxTokenSize = null
                null, // logPrefix = null
                null, // resultSetNameTemplate = null
                null, // maxBatchSize = null
                null  // backpressureStrategy = null
        );

        float[] mockEmbedding = new float[]{0.1f, 0.2f, 0.3f};

        when(objectMapper.readValue(anyString(), eq(EmbedderOptions.class))).thenReturn(optionsWithNulls);
        when(vectorizer.embeddings(anyString())).thenReturn(Uni.createFrom().item(mockEmbedding));
        lenient().when(vectorizer.getModelId()).thenReturn("ALL_MINILM_L6_V2");
        lenient().when(vectorizer.getModel()).thenReturn(EmbeddingModel.ALL_MINILM_L6_V2);
        lenient().when(vectorizer.isUsingGpu()).thenReturn(false);
        ...
        // Should succeed because ensureDefaults() method handles null fields
        assertTrue(response.getSuccess());
        assertTrue(response.getProcessorLogs(0).contains("Successfully processed document fields"));
